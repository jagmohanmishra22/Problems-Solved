...
class TestClass {
	...
	private static Node findLCA(Node root, int k1, int k2) {
		...
		else if(root.key==k1 ||  root.key==k2) return root;//When one of the key is root itself
		else
		{
			Node left=findLCA(root.left,k1,k2);
			Node right=findLCA(root.right,k1,k2);
			if(left==null && right ==null)return null;
			else if(left==null && right !=null)return right;//LCA exists in RST
			else if(left!=null && right ==null)return left;//LCA exists in LST
			else return root;
		}
	}
